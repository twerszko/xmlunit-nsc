...
public class ElementComparisonProviderTest {
	...
	private List<Comparison> testAttributeSequence(XmlUnitProperties properties) throws Exception {
		...
		ElementComparisonProvider strategy = new ElementComparisonProvider(properties.getIgnoreAttributeOrder());
	...
	private List<Comparison> findElementDifferences(Element controlNode, Element testNode) {
		...
		Comparisons comparisons = new ElementComparisonProvider(false).provideComparisons(control, test);
		comparator.executeComparisons(comparisons);
		return comparator.getDifferences();
	}
}
