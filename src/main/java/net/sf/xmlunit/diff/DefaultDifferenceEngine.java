...
public final class DefaultDifferenceEngine extends DOMDifferenceEngine {
    ...
    // TODO compare with compareNodes from parent class
    public void compare(Node control, Node test) {
        Source ctrlSource = Input.fromNode(control).build();
        Source testSource = Input.fromNode(test).build();
        compare(ctrlSource, testSource);
    }
