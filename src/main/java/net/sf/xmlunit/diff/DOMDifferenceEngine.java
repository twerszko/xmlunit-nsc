...
import net.sf.xmlunit.diff.strategies.CompareNodeStrategy;
import net.sf.xmlunit.diff.strategies.ComparisonStrategy;
import net.sf.xmlunit.diff.strategies.ComparisonStrategyBase;
...
public final class DOMDifferenceEngine extends AbstractDifferenceEngine {
	...
	void compareNodes(NodeAndXpath<Node> control, NodeAndXpath<Node> test) {
		new CompareNodeAndChildrenStrategy(getComparisonPerformer()).compare(control, test);
	}
	 ...
	 * </p>
	 */
	private class CompareNodeAndChildrenStrategy extends ComparisonStrategyBase<Node> {

		public CompareNodeAndChildrenStrategy(ComparisonPerformer compPerformer) {
			super(compPerformer);
		}
		...
		private void executeChildrenComparison(NodeAndXpath<Node> control, NodeAndXpath<Node> test) {
			...
			Iterable<Node> controlChildren = getFilteredChildNodes(control.getNode());
			Iterable<Node> testChildren = getFilteredChildNodes(test.getNode());
		...
		private void executeRecursion(NodeAndXpath<Node> controlChild, NodeAndXpath<Node> testChild) {
			ComparisonStrategy<Node> strategy = new CompareNodeAndChildrenStrategy(performer);
			strategy.compare(controlChild, testChild);
			setInterrupted(strategy.isInterrupted());
		}
		...
		protected void compareInternal(NodeAndXpath<Node> control, NodeAndXpath<Node> test) {
			// TODO
			Queue<Comparison> comparisons = new LinkedList<Comparison>();
			comparisons.addAll(new CompareNodeStrategy(performer, properties.getIgnoreAttributeOrder())
